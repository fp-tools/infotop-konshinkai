...
recend-worker v2 のエンドポイント: `POST /`（即時送信）／`POST・GET・DELETE /schedule`（予約配信）／`GET /opens`（開封一覧）／`GET /open?mid=`（開封計測ピクセル・認証なし）。QRコード画像はメール内でJavaScriptが使えないため、外部QR画像API（api.qrserver.com）のURL埋め込みで表示している。
