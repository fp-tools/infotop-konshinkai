...
/* ==================== QRコード画像生成（自前ホスティング・外部API非依存） ====================
 * GET /qr?data=...&size=220 → PNG を返す（認証なし。メール内 <img> から参照される）
 * QRエンコード: qrcode-generator v1.4.4 (MIT License, Kazuhiko Arase) を同梱
 * PNG化: グレースケールPNGを自前で組み立て（zlib圧縮は Workers 標準の CompressionStream を使用）
 */
...
/** 8bitグレースケールPNGを生成する（pixels: dim*dim, 0=黒 255=白） */
async function pngGray(dim, pixels) {
  const raw = new Uint8Array((dim + 1) * dim); // 行頭に filter byte(0)
  for (let y = 0; y < dim; y++) raw.set(pixels.subarray(y * dim, (y + 1) * dim), y * (dim + 1) + 1);
  const ihdr = new Uint8Array(13);
  const dv = new DataView(ihdr.buffer);
  dv.setUint32(0, dim); dv.setUint32(4, dim);
  ihdr[8] = 8; ihdr[9] = 0; // bit depth 8 / grayscale
  const idat = await zlibDeflate(raw);
  const parts = [new Uint8Array([137, 80, 78, 71, 13, 10, 26, 10]), pngChunk('IHDR', ihdr), pngChunk('IDAT', idat), pngChunk('IEND', new Uint8Array(0))];
  const out = new Uint8Array(parts.reduce((s, p) => s + p.length, 0));
  let o = 0; for (const p of parts) { out.set(p, o); o += p.length; }
  return out;
}
async function qrPng(text, size) {
  const qr = qrcode(0, 'M');
  qr.addData(text);
  qr.make();
  const n = qr.getModuleCount();
  const quiet = 4; // 標準の静穏帯（4モジュール）
  ...
  for (let r = 0; r < n; r++) for (let c = 0; c < n; c++) {
    ...
    for (let y = y0; y < y0 + scale; y++) px.fill(0, y * dim + x0, y * dim + x0 + scale);
  }
...
export default {
  async fetch(request, env, ctx) {
    ...
    // --- QRコード画像（認証なし・メールの<img>から参照される） ---
    if (request.method === 'GET' && path === '/qr') {
      const data = url.searchParams.get('data') || '';
      if (!data || data.length > 512) return new Response('bad request', { status: 400, headers: CORS });
      const size = Math.min(1024, Math.max(64, parseInt(url.searchParams.get('size') || '220', 10) || 220));
      try {
        const png = await qrPng(data, size);
        return new Response(png, { status: 200, headers: { ...CORS, 'Content-Type': 'image/png', 'Cache-Control': 'public, max-age=31536000, immutable' } });
      } catch (e) {
        return new Response('qr error: ' + String(e), { status: 500, headers: CORS });
      }
    }
